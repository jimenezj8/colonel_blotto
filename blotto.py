...
class BlottoRound:
    ...
    @fields.setter
    def fields(self):
        logger.info("Number of fields for a configuration cannot be modified.")
        return self._fields
    ...
    @soldiers.setter
    def soldiers(self):
        logger.info("Number of soldiers for a configuration cannot be modified.")
        return self._soldiers
...
class DecreasingSoldiers(BlottoRound):
    ...
    def __init__(self, fields: Union[int, None] = None, soldiers: Union[int, None] = None):
        '''
        ...
        if fields is None and soldiers is None:
            ...
            raise ValueError("If one of [fields, soldiers] is provided, both must be provided.")
    ...
    def check_field_rules(self, submission: List[int]):
        ...
        for field, soldiers in enumerate(submission):
            if type(soldiers) is not int:
                raise ValueError(f"Error on Field {field}: all submissions must be integer values")
            if soldiers < 0:
                raise ValueError(f"Error on Field {field}: all submissions must be positive values")
            if field > 0:
                if soldiers > submission[field-1]:
                    raise ValueError(f"Error on Field {field}: decreasing soldiers criteria not met")

    def score_opponents(self, submission_one: Dict[str, Union[str, List]], submission_two: Dict[str, Union[str, List]]):
        '''
...
class RoundLibrary:
    ...
    ROUND_MAP = {
        DecreasingSoldiers.ID: DecreasingSoldiers
    }
